...
from ..utils.html_utils import URLDownloader
...
class ENSO_Indices( object ):
  """ 
  Class to download various ENSO indices and place them all in a single netCDF file.
  ...
  nino34_1  = URLS['ENSO']['base'] + URLS['ENSO']['nino34_1']                 # ESRL/NOAA, Nino 3.4, ascii text (1948-present)*
  nino34_2  = URLS['ENSO']['base'] + URLS['ENSO']['nino34_2']                 # ESRL/NOAA Nino 3.4, 1870-present based on HadISST*
  nino4     = URLS['ENSO']['base'] + URLS['ENSO']['nino4']                    # ESRL/NOAA, Nino 4, ascii text (1948-present)*
  nino12    = URLS['ENSO']['base'] + URLS['ENSO']['nino12']                   # ESRL/NOAA, Nino 1+2, ascii text (1948-present)*
  oni       = URLS['ENSO']['base'] + URLS['ENSO']['oni']                      # ESRL/NOAA, Ocean Nino Index, ascii text (1950-present)*
  tni       = URLS['ENSO']['base'] + URLS['ENSO']['tni']                      # ESRL/NOAA, Trans Nino Index, ascii text (1948-present)*
  mei       = URLS['ENSO']['base'] + URLS['ENSO']['mei']                      # ESRL Multivariate ENSO Index (1950 - present)**
  ...
  def getNino34_HadISST(self):
    """Download Nino34_HadISST data and convert to standard format for netCDF"""
  ...
  def download( self, url ):
    with URLDownloader( url ) as resp:
      charset = resp.resp.info().get_content_charset()
      data    = resp.read()
...
def getENSO(outRoot):
  """Wrapper function for creating ENSO_Indices instance and downloading"""

  outDir  = getOutDir( outRoot )
  outFile = os.path.join(outDir, 'ENSO_Indices.nc')
  enso    = ENSO_Indices()
  enso.saveData( outFile, enso.allData() )
